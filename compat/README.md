...
1. **Committed vectors** (`vectors/*.json`) — deterministic test vectors
   ...
   libsignal **v0.96.4**) and committed to the repo. `vectors_test.go` consumes
...
for d in curve kem-decaps hkdf messages fingerprint sessions groups sealedsender account-keys username-links; do
...
`account-keys` or `username-links` when the upstream pin changes.
...
between the Go session layer and the genuine upstream session API (v0.96.4) over
...
v0.96.4 sessions are **PQXDH/v4 only**. X3DH/v3 is removed upstream: every
...
The v4 interop above is the full session surface v0.96.4 supports. At v0.96.4
...
is covered; it simply cannot be *cross-checked* against the v0.96.4 harness,
