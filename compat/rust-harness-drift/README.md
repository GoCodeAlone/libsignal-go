...
instead of the `v0.96.4` pin. Its sole job is to feed the weekly
...
the committed `v0.96.4` vectors, so a byte-level change to a primitive we rely
...
The pinned harness is the behavioral **contract** (`v0.96.4`, ADR 0001 Stage 2). It must
